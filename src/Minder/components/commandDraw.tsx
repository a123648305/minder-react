...
const CommandList: React.FC<PropsType> = () => {
  const component = commandGroup.map(({ items, groupName }) => {
    const groupDom = (
      <li
        className={classNames(styles.commandDraw_item, styles.commandDraw_title)}
        key={groupName + 'li'}
      >
        {groupName}
      </li>
    ...
    const kesDom = items.map((item) => (
      <li className={styles.commandDraw_item} key={item.label}>
        ...
        <div className={styles.command_item_kyes}>{keysDom(item.commandkey, item.label)}</div>
      </li>
    ));
    return (
    ...
    <>
      ...
      <div className={styles.camand_popover}>
        <Popover content="帮助" placement="top">
          <i className="icon iconfont icon-wenjuan" onClick={() => SetOpen(true)}></i>
        </Popover>
      </div>
    </>
  );
